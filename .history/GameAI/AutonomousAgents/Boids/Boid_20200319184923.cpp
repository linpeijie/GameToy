...
Vector2D Boid::wander()
{
    ...
    double randomClamped = (rand())/(RAND_MAX+1.0) * 2 - 1;
    // 1.对目标添加随机位移
    double JitterThisTimeSlice = wanderJitter * this->elapsed;
    //wanderTarget.subVector(Vector2D(u(e) * JitterThisTimeSlice, u(e) * JitterThisTimeSlice));
    wanderTarget.subVector(Vector2D(randomClamped * JitterThisTimeSlice, randomClamped * JitterThisTimeSlice));
    cout << randomClamped * JitterThisTimeSlice << endl;
    // 2.对目标标准化后，重新投影到圆边上
