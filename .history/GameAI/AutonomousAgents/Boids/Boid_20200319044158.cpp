...
Boid::Boid(float x, float y)
{
    ...
    cout << wanderTarget.x << " mmm " << wanderTarget.y << endl;
}
...
Vector2D Boid::wander()
{
    ...
    cout << wanderTarget.x << " aaa " << wanderTarget.y << endl;
    // 2.对目标标准化后，重新投影到圆边上
    wanderTarget.normalize();
    wanderTarget.mulScalar(wanderRadius);
    cout << wanderTarget.x << " bbb " << wanderTarget.y << endl;
    // 3.向前移动目标
    wanderTarget.addVector(Vector2D(wanderDistance, 0));
    // 4.把目标投影到世界空间
    wanderTarget.addVector(location);
    cout << wanderTarget.x << " xxx " << wanderTarget.y << endl;
    // 4.计算出智能体到达目标位置的力
