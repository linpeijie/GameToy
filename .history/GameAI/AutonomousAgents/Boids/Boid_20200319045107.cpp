...
Vector2D Boid::seek(Vector2D v)
{
    ...
    cout << desired.x << " aaa " << desired.y << endl;
    cout << velocity.x << " vvv " << velocity.y << endl;
    // 移动所需的速度 = 预期速度 - 当前速度
    acceleration.subTwoVector(desired, velocity);
    cout << acceleration.x << " bbb " << acceleration.y << endl;
    // 限制力不要超过最大力
    acceleration.limit(maxForce);
    cout << acceleration.x << " ccc " << acceleration.y << endl;
